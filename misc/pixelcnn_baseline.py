...
train_loader = torch.utils.data.DataLoader(eval('datasets.'+ DATASET)
                                           ( f'../data/{DATASET}/', train=True, download=True, transform=transforms.ToTensor(),), 
...
test_loader = torch.utils.data.DataLoader(eval('datasets.'+DATASET)
                                          ( f'../data/{DATASET}/', train=False, transform=transforms.ToTensor(),), 
                                          batch_size=BATCH_SIZE, 
                                          shuffle=False,
                                          num_workers=NUM_WORKERS, 
                                          pin_memory=True
    ...
    for batch_idx, (x, label) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % PRINT_INTERVAL == 0:
            print(f"""\tIter: [{batch_idx * len(x)}/{len(train_loader.dataset)} 
                  ...
                  \tLoss: {np.asarray(train_loss)[-PRINT_INTERVAL:].mean(0),} 
...
def test():
    ...
    print(f'Validation Completed!\tLoss: {np.asarray(val_loss).mean(0)} Time: {time.time() - start_time}')
...
def generate_samples():
    ...
    save_image(images[:, None], f'samples/pixelcnn_baseline_samples_{DATASET}.png', nrow=10)
...
for epoch in range(1, N_EPOCHS):
    ...
    if ALWAYS_SAVE or cur_loss <= BEST_LOSS:
        ...
        print("Saving model!")
        torch.save(model.state_dict(), f'models/{DATASET}_pixelcnn.pt')
    else:
